...
public class ConnectViaDuplicationEditor : Editor
{
	...
	bool changed=false;
		...
		ConnectViaDuplication.mode old = myTarget.choice;
				myTarget.choice = (ConnectViaDuplication.mode)EditorGUILayout.EnumPopup ("Mode of creation: ", myTarget.choice);
		if(myTarget.choice!=old){
			changed=true;
		}
		...
		if(spacing!=myTarget.spacing){
			changed=true;
				...
				if (myTarget.choice == ConnectViaDuplication.mode.chooseNumber) {
						EditorGUILayout.Space ();
						EditorGUILayout.Space ();
		...
		if(oldNum!=myTarget.number){
			...
			if(changed)
				...
				if (myTarget.choice == ConnectViaDuplication.mode.chooseSpacing) {
		...
		void refresh ()
		{
				...
				//float startGap=0;
						...
						float startGap = Vector3.Distance (myTarget.startPoint, myTarget.endPoint) % (gap * myTarget.number);
						if (myTarget.endPoint.x < myTarget.startPoint.x) {
								x = oldPos.x + ((Mathf.Cos (angle) * startGap / 2));
						} else {
								x = oldPos.x - ((Mathf.Cos (angle) * startGap / 2));
