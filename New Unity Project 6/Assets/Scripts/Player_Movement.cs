...
public class Player_Movement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!movementDisabled)
        {
            ...
            else if (Physics2D.Linecast(transform.position, leftWallCheck[1].position, 1 << LayerMask.NameToLayer("level")))
            {
                clinging = true;
                onWall = true;
                dubjump = false;
    ...
    void FixedUpdate()
    {
        if (!movementDisabled)
        {
            ...
            if (WallJumping)
            {
                if (leftWallJump)
                {
                    this.wallJump(wallJumpAngle, true);
                }
                else
                {
                    this.wallJump(wallJumpAngle, false);
                }
                WallJumping = false;
            }
            if (willJump)
